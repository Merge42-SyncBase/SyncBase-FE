...
The mark compresses SyncBase's product promise into two fields and the seam between them:
...
| `public/brand/syncbase-mark-{32,64,128,512}.png` | Raster mark exports at common delivery sizes |
| `public/brand/syncbase-logo-horizontal-1600.png` | Large horizontal raster lockup |
| `public/brand/syncbase-brand-sheet.png` | Shareable raster brand sheet |
...
The logo deliberately stays two-color. Verified Green remains available for literal ACTIVE and successful product states, but it is not part of the brand mark.

## Usage rules

- Keep clear space equal to at least one eighth of the mark's width on all sides.
- Use the primary mark at 16px or larger. Prefer the monochrome mark when a process requires one-color reproduction.
...
- Do not rotate the symbol or animate the seam as a generic sync loop. If animated later, the only faithful motion is a single trace traveling from the active field to the base.
